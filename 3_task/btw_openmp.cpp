...
void alloc_mem(size_t rprt_n, predecessors *&P, int *&sigma, int *&d, float *&delta){
...
void free_mem(predecessors *&P, int *&sigma, int *&d, float *&delta){
	free(P);
	free(sigma);
	free(d);
	free(delta);
	...
	for(size_t s = 0; s < rprt_n; s++){
		...
		while(!Q.empty()){
			...
			for(size_t i = 0; i < neig_num; i++){ 			// w = neighbours[i]
				...
				if(d[w] == d[v] + 1){
					// #pragma omp critical
					// printf("s %d sigma[w] %d sigma[v] %d\n", s, sigma[w], sigma[v]);
					sigma[w] += sigma[v];
